	...
	u32 bl_lvl)
{
	...
	if (bl_lvl > 0) {
		if (panel->aod_layer || (panel->power_mode == SDE_MODE_DPMS_LP1 || panel->power_mode == SDE_MODE_DPMS_LP2)) {
			pr_info("MSM_LCD backlight donot set pm %d, aod_layer %d\n", panel->power_mode, panel->aod_layer);
			goto exit;
		}
