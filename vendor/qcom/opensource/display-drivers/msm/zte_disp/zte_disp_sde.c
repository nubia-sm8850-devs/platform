...
void process_layer(struct dsi_panel *panel, u32 layer) {
    ...
    if (panel->aod_layer != aod_layer) {
       panel->aod_layer = aod_layer;
       zte_aod_handler(panel, aod_layer ? FRAME_AOD_ACTIVE : FRAME_AOD_MISS);
    }

    if (panel->fod_layer != fod_layer) {
       panel->fod_layer = fod_layer;
       zte_fod_handler(panel, fod_layer ? FOD_ACTIVE : FOD_MISS);
    }
}
/*
